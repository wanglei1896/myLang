...
let rec exec stmt (locEnv : locEnv) (gloEnv : gloEnv) (store : store) : store = 
    ...
    | Switch(e, cases) ->                         (*mmmmmmmm*)
      let (v, store1) = eval e locEnv gloEnv store
      let rec loop v ss (locEnv, store) =
          match ss with
          | [ ] -> store
          | s1::sr -> if (fst s1)=v then exec (snd s1) locEnv gloEnv store
                                    else loop v sr (locEnv, store)
      loop v cases (locEnv, store1)

    | DoWhile(e, body) ->                         (*mmmmmmmm*)
      let rec loop store1 =
              let store2 = exec body locEnv gloEnv store1
              let (v, store3) = eval e locEnv gloEnv store2
              if v<>0 then loop (store3)
                      else store3
      loop store
     
    | For(e1, e2, e3, body) ->                    (*mmmmmmmm*)
    ...
    | Break -> failwith "return not implemented"  (*mmmmmmmmmm*)
...
and eval e locEnv gloEnv store : int * store = 
    ...
    | Assign(acc, e) -> let (loc, store1) = access acc locEnv gloEnv store
                        ...
                        (res, setSto store2 loc res) 
                                                           (*mmmmmmmmmmmm*)
    | AssPP acc      -> let (loc, store1) = access acc locEnv gloEnv store
                        let res = getSto store1 loc + 1
                        (res, setSto store1 loc res)
    | AssDD acc      -> let (loc, store1) = access acc locEnv gloEnv store
                        let res = getSto store1 loc - 1
                        (res, setSto store1 loc res)
    ...
    | Prim3(ope, e1, e2, e3) ->
      let res =
          match ope with
          | ("?",":") -> let (i1, store1) = eval e1 locEnv gloEnv store
                         if i1<>0 then eval e2 locEnv gloEnv store1
                         else eval e3 locEnv gloEnv store1
          | _         -> failwith ("unknown primitive " + fst ope + snd ope)
      res
