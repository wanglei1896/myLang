...
let rec cStmt stmt (varEnv : varEnv) (funEnv : funEnv) : instr list = 
    ...
    | If(e, stmt1, stmt2) -> 
      ...
      @ [Label labend]
                                                               (*mmmmmmmmmmm*)
    | Switch(e,cases) ->
      let rec loop cases varEnv =
          match cases with
          | []      -> []
          | s1::sr  -> let labother = newLabel()
                       cExpr e varEnv funEnv
                       @ [CSTI (fst s1)] @ [EQ; IFZERO labother]
                       @ cStmt (snd s1) varEnv funEnv
                       @ [Label labother] @ loop sr varEnv
      loop cases varEnv
                                                               (*mmmmmmmmmmm*)
    | For(estart, e, estop, body) ->
      let labbegin = newLabel()
      let labtest  = newLabel()
      cExpr estart varEnv funEnv @ [INCSP -1]
...
and cExpr (e : expr) (varEnv : varEnv) (funEnv : funEnv) : instr list = 
    ...
    | Assign(acc, e) -> cAccess acc varEnv funEnv @ cExpr e varEnv funEnv @ [STI]
                                                                              (*mmmmmmmmmmm*)
    | AssPP  acc     -> cAccess acc varEnv funEnv @ [DUP] @ [LDI] @ [CSTI 1] @ [ADD] @ [STI]
    | AssDD  acc     -> cAccess acc varEnv funEnv @ [DUP] @ [LDI] @ [CSTI 1] @ [SUB] @ [STI]
